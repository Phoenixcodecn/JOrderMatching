...
public class FIXTradeApplication extends AbstractFIXServer {
  ...
  public FIXTradeApplication(OrderMatchingEngine engine, FIXServerConfiguration configuration,
      IElementQueue<FIXMessage> queue, boolean verbose) {
          ...
          switch (msg.getMessageType()) {
            case NEW_ORDER:
              ...
              if (order.getOpenQty() != 0) {
                IFIXMessage _msg = createExecutionReport(session, order, generator.genExecutionID(),
                    FIXOrderStatus.NEW);
                _msg.send();
  ...
  /*
   * @Override public void onTraderLogon(FIXSession session, String username) { if
   * (verbose) {
   * System.out.println(this.getClass().getSimpleName()+"::onTraderLogon " +
   * session + " " + username); } }
   */
  ...
  public void onFixMessage(FIXMessage incomingMsg, FIXSession session) {
    if (verbose) {
      String s = incomingMsg.toString();
      s = s.replace(FIXConst.FIX_DELIMITER, '|');
      System.out.println(this.getClass().getSimpleName() + "::onFixMessage: " + s);
    }
    switch (incomingMsg.getMessageType()) {
      case NEW_ORDER:
        ...
        break;
      case ORDER_CANCEL:
        createExecutionReport(session, getOrderFromCancelReplaceRequest(incomingMsg), generator.genExecutionID(),
            FIXOrderStatus.PENDING_CANCEL);
        addToMessageHandlingQueue(incomingMsg, session);
        break;
      case ORDER_CANCEL_REPLACE:
        createExecutionReport(session, getOrderFromCancelReplaceRequest(incomingMsg), generator.genExecutionID(),
            FIXOrderStatus.PENDING_REPLACE);
        addToMessageHandlingQueue(incomingMsg, session);
        break;
      default:
        break;
  ...
  public class FIXTradeHandler {
    ...
    public List<IFIXMessage> handleNewOrderSingle(FIXSession session, Order order) {
      try {
        ...
        for (Order _order : matchedList) {
          ...
          cumQty += lastShares;
          replyList.add(createExecutionReport(getSession(_order.getSender()), _order,
              _order.getOpenQty() == 0L ? FIXOrderStatus.FILLED : FIXOrderStatus.PARTIALLY_FILLED, execID,
              _order.getOpenQty(), _order.getExecutedQty(), _order.getAvgExecutedPrice(), _order.getLastExecutedQty(),
              _order.getLastExecutedPrice()));
          replyList.add(createExecutionReport(session, order,
              leavesQty == 0L ? FIXOrderStatus.FILLED : FIXOrderStatus.PARTIALLY_FILLED, execID, leavesQty, cumQty,
              avgExecPx, lastShares, lastPx));
        }
    ...
    public IFIXMessage handleOrderCancelReplaceRequest(FIXSession session, FIXMessage message) {
      ...
      final Side side = message.getValueAsChar(FIXConst.TAG_ORDER_SIDE) == FIXConst.ORDER_SIDE_BUY ? Side.BUY
          : Side.SELL;

      String error = engine.modifyOrder(symbol, side, internalOrderID, newInternalOrderID,
          message.getValueAsLong(FIXConst.TAG_ORDER_QUANTITY), message.getTagValueAsDouble(FIXConst.TAG_ORDER_PRICE));
      if (error == null) {
  ...
  public static IFIXMessage createExecutionReport(FIXSession session, Order order, final String execID,
      FIXOrderStatus ordStatus) {
    return createExecutionReport(session, order, ordStatus, execID, order.getOpenQty(), order.getExecutedQty(),
        order.getAvgExecutedPrice(), order.getLastExecutedQty(), order.getLastExecutedPrice());
  }

  public static FIXExecutionReportMessage createExecutionReport(final FIXSession session, final Order order,
      final FIXOrderStatus ordStatus, final String execID, final long leavesQty, final long cumQty, final double avgPx,
      final long lastShares, final double lastPx) {

    char execType = ordStatus == FIXOrderStatus.FILLED || ordStatus == FIXOrderStatus.PARTIALLY_FILLED ? 'F'
        /* Trade */ : ordStatus.value;
    ...
    return message;
    /*
     * try { session.send(message); } catch (IOException e) { e.printStackTrace(); }
     */
  }

  private FIXMessage createOrderCancelRejectMessage(FIXSession session, String clOrdID, String origClOrdID,
      String text) {
